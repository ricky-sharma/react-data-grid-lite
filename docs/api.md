...
| `draggable`     | `boolean`           | Enables or disables dragging for an individual column. Overrides the global `enableColumnDrag` setting. Fixed columns can be reordered among themselves, and non-fixed columns within their own group. When enabled, it also adds a “Move” menu item to allow moving the column left or right — in addition to drag-and-drop functionality.|       -       |  No          |
...
| `enableColumnDrag`   | `boolean` | Enables column dragging for all columns. Individual column `draggable` settings override this option. Fixed columns can be reordered among themselves, and non-fixed columns within their own group. When enabled, a “Move” menu item is also added, allowing columns to be moved left or right — in addition to drag-and-drop functionality.|      `true`         | No           |
